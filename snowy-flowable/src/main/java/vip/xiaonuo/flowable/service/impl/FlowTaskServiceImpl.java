package vip.xiaonuo.flowable.service.impl;
...
import vip.xiaonuo.flowable.common.constant.ProcessConstants;
import vip.xiaonuo.flowable.common.enums.FlowComment;
import vip.xiaonuo.flowable.domain.DO.SysForm;
import vip.xiaonuo.flowable.domain.dto.FlowCommentDto;
import vip.xiaonuo.flowable.domain.dto.FlowNextDto;
import vip.xiaonuo.flowable.domain.dto.FlowTaskDto;
import vip.xiaonuo.flowable.domain.dto.FlowViewerDto;
import vip.xiaonuo.flowable.domain.vo.FlowQueryVo;
import vip.xiaonuo.flowable.domain.vo.FlowTaskVo;
import vip.xiaonuo.flowable.factory.FlowServiceFactory;
import vip.xiaonuo.flowable.flow.CustomProcessDiagramGenerator;
import vip.xiaonuo.flowable.flow.FindNextNodeUtil;
import vip.xiaonuo.flowable.flow.FlowableUtils;
import vip.xiaonuo.flowable.service.IFlowTaskService;
...
//        taskService.setOwner(flowTaskVo.getTaskId(), flowTaskVo.getAssignee());
        ...
        for (HistoricProcessInstance hisIns : historicProcessInstances) {
            ...
            if (CollectionUtils.isNotEmpty(taskList)) {
                ...
                if (StringUtils.isNotBlank(taskList.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    fillLoginUserInfo(taskList.get(0).getAssignee(), flowTask);
                }
            } else {
                ...
                if (StringUtils.isNotBlank(historicTaskInstance.get(0).getAssignee())) {
                    // 当前任务节点办理人信息
                    fillLoginUserInfo(historicTaskInstance.get(0).getAssignee(), flowTask);
                }
            }
    ...
    private void fillLoginUserInfo(String userId, FlowTaskDto flowTask) {
        SysLoginUser sysUser = sysUserService.getUserById(userId);
        if (Objects.nonNull(sysUser)) {
            flowTask.setAssigneeId(Long.valueOf(sysUser.getId()));
            flowTask.setAssigneeName(sysUser.getNickname());
            flowTask.setAssigneeDeptName(Objects.nonNull(sysUser.getOrgId()) ? sysUser.getOrgName() : "");
        }
    }
...
//      Map<String, VariableInstance> variables = runtimeService.getVariableInstances(currentTask.getExecutionId());
    ...
    public AjaxResult todoList(FlowQueryVo queryVo) {
        ...
        TaskQuery taskQuery = taskService.createTaskQuery()
                ...
                .taskCandidateOrAssigned(loginUser.getId())
...
//        }
        ...
        for (Task task : taskList) {
            ...
            fillLoginUserInfo(historicProcessInstance.getStartUserId(), flowTask);
            flowList.add(flowTask);
        }
...
//                } else
                if (StringUtils.isNotBlank(histIns.getTaskId())) {
                    ...
                    if (StringUtils.isNotBlank(histIns.getAssignee())) {
                        fillLoginUserInfo(histIns.getAssignee(), flowTask);
                    }
                    // 展示审批人员
                    ...
                    for (HistoricIdentityLink identityLink : linksForTask) {
                        ...
                        if ("candidate".equals(identityLink.getType())) {
                            if (StringUtils.isNotBlank(identityLink.getUserId())) {
                                SysLoginUser sysUser = sysUserService.getUserById(identityLink.getUserId());
